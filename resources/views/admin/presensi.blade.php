        ...
        <div class="w-full overflow-x-auto max-w-full mt-5 p-5 bg-white dark:bg-gray-800 rounded-sm shadow-xl">
            <div class="flex flex-col md:flex-row">
                <div class="flex flex-col w-full mb-4 md:w-1/2 mr-0 md:mr-8">
                    <label for="filter-presensi" class="mb-1 font-semibold text-gray-800 dark:text-gray-200">Filter Data
                        Perkuliahan:</label>
                    <select id="filter-presensi" name="prodi_id"
                        class="bg-white dark:bg-gray-700 dark:text-white border border-gray-300 dark:border-gray-600 rounded-sm px-2 py-2">
                ...
                <div class="flex flex-col w-full mb-4 md:w-1/2 mr-0">
                    <label for="filter-status" class="mb-1 font-semibold text-gray-800 dark:text-gray-200">Filter Status
                        Perkuliahan:</label>
        ...
        <div class="w-full overflow-x-auto max-w-full mt-5 p-5 bg-white dark:bg-gray-800 rounded-sm shadow-xl">
            <div class="mb-10 flex">
                <a href="{{ route('admin.presensi.create') }}">
                    <button
                        class="flex items-center px-4 py-2.5 text-white bg-blue-600 hover:bg-blue-700 active:bg-blue-800 rounded-sm font-semibold cursor-pointer">
            ...
            <div class="overflow-x-auto w-[340px] sm:w-150 md:w-full mt-3 pb-3">
                <table id="data-presensi" class="text-sm min-w-full display nowrap pt-1 dark:text-white">
                    ...
                    <tbody>
                        @foreach ($presensi as $p)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-600">
                                <td class="border-gray-300 dark:border-gray-600 dark:text-white px-4 py-2">
                                    {{ $p->tgl_presensi ?? '-' }}</td>
                                <td class="border-gray-300 dark:border-gray-600 dark:text-white px-4 py-2">
                                    {{ substr($p->jam_awal, 0, 5) . ' - ' . substr($p->jam_akhir, 0, 5) }}</td>
                                <td class="border-gray-300 dark:border-gray-600 dark:text-white px-4 py-2">
                                    {{ $p->pertemuan->matkul->nama_matkul }}</td>
                                <td class="border-gray-300 dark:border-gray-600 dark:text-white px-4 py-2 text-center">
                                    {{ $p->pertemuan->pertemuan_ke ?? '-' }}</td>
                                <td class="border-gray-300 dark:border-gray-600 dark:text-white px-4 py-2">
                                    {{ $p->dosen->nama }}</td>
                                <td class="border-gray-300 dark:border-gray-600 dark:text-white px-4 py-2">
                                    {{ $p->pertemuan->prodi->nama_prodi }}</td>
                                <td class="border-gray-300 dark:border-gray-600 dark:text-white px-4 py-2 text-center">
                                    {{ $p->pertemuan->semester }}</td>
                                ...
                                <td class="border-gray-300 dark:border-gray-600 dark:text-white px-4 py-2 text-center">
                                    <div class="flex justify-center gap-2">
                                        <a href="{{ route('admin.presensi.edit', $p->id) }}">
                                            <button
                                                class="cursor-pointer px-2 py-1 bg-yellow-600 hover:bg-yellow-700 active:bg-yellow-800 text-white rounded-md">
                                        ...
                                        <form action="{{ route('admin.presensi.destroy', $p->id) }}" method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="px-2 py-1 bg-red-600 hover:bg-red-700 active:bg-red-800 text-white rounded-md">

<x-layout>
    @vite(['resources/js/pages/dosen/rekap-matkul.js'])
    <div class="h-full dark:bg-darkCard">
        ...
        <div class="w-full overflow-x-auto max-w-full mt-5 p-5 bg-white dark:bg-gray-800 rounded-sm shadow-xl">
            <form action="{{route( 'dosen.rekap-matkul.filter')}}" method="post">
                @csrf
                ...
                <div class="w-full flex justify-end mt-3">
                    <a href="{{route('dosen.rekap-matkul.index')}}" class="px-5 py-2 mr-2 bg-red-500 hover:bg-red-600 active:bg-red-700 text-white font-semibold rounded-md cursor-pointer">Reset</a>
        ...
        <div class="w-full mt-5 p-5 bg-white dark:bg-gray-800 rounded-sm shadow-xl">
            @if ($prodiTerpilih && $matkulTerpilih && $semesterTerpilih)
                <div class="mt-4 flex gap-4">
                    ...
                    <form action="{{ route('dosen.export.matkul.pdf') }}" method="POST">
                        @csrf
                        <input type="hidden" name="prodi" value="{{ request('prodi') }}">
                        <input type="hidden" name="semester" value="{{ request('semester') }}">
                        <input type="hidden" name="matkul" value="{{ request('matkul') }}">

    ...
    <div class="h-[680px]">
        ...
        <div class="w-full mt-5 p-5 bg-white dark:bg-gray-800 rounded-sm shadow-xl">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 h-full">
                <div
                    class="bg-gray-200 dark:bg-[#1e293b] rounded-md p-6 shadow-md flex flex-col items-center justify-center text-center h-full">
                    <div class="flex flex-col items-center">
                        ...
                        @if ($presensi)
                            @if ($presensiTercatat)
                                ...
                                <p class="text-lg font-semibold text-green-600 dark:text-green-400">✅ Presensi Tercatat
                                </p>
                                <p class="mt-2 text-gray-800 dark:text-gray-200 font-medium">Mata Kuliah:
                                    {{ $presensi->pertemuan->matkul->nama_matkul }}
                                </p>
                                <p class="text-gray-600 dark:text-gray-400">Ruangan:
                                    {{ $presensi->ruangan->nama_ruangan ?? '-' }}</p>
                        ...
                        @else
                            <div class="text-center text-gray-600 dark:text-gray-400">
                                ...
                                <p class="text-sm">Anda dapat memeriksa kembali jadwal akademik Anda untuk informasi
                                    lebih lanjut.</p>
                            </div>
                        @endif
                    </div>
                                ...
                                <div class="flex justify-between items-center">
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-600 dark:text-gray-300">Jam Kuliah:
                                            {{ substr($r->jam_awal, 0, 5) . ' - ' . substr($r->jam_akhir, 0, 5) }}</p>
                                        <p class="text-sm text-gray-600 dark:text-gray-300">Presensi Masuk:
                                            {{ $r->waktuPresensi ?? '-' }}</p>
                                        {{-- <p class="text-sm text-gray-600 dark:text-gray-300">Presensi Masuk:
                                            {{ $r->detailPresensi->first()->waktu_presensi ?? '-' }}</p> --}}

                                        {{-- Keterangan surat ditolak --}}
                                        @if ($r->keteranganSurat)
                                            <p class="text-xs text-red-500 mt-1">{{ $r->keteranganSurat }}</p>
                                        @endif
                                    </div>

                                    <div class="{{ $r->colorPresensi }}">
                                        {{ $r->labelPresensi }}{{ $r->labelPertemuan }}
                                    </div>
                                </div>
                            </div>
                        @empty
                                    ...
                                    @php
                                        ...
                                        switch ($statusPresensi) {
                                            ...
                                            case '3':
                                                ...
                                                break;
                                            case '4':
                                                $color = 'text-orange-500 dark:text-orange-400 font-semibold text-sm';
                                                $labelPresensi = '⏳ Menunggu Konfirmasi Surat';
                                                break;
                                            case '0':
